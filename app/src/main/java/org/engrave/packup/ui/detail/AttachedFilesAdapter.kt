...
) : RecyclerView.Adapter<AttachedFilesAdapter.ItemViewHolder>(
) {
    open class ItemViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView)
    class FileViewHolder(itemView: View) : ItemViewHolder(itemView) {
            ...
            FILE_ITEM -> {
                FileViewHolder(
                    LayoutInflater.from(parent.context)
                        .inflate(R.layout.item_file_blobs, parent, false)
                )
            }
            else -> ItemViewHolder(
                LayoutInflater.from(parent.context)
                    .inflate(R.layout.item_file_spacing, parent, false)
            )
        }

    override fun getItemViewType(position: Int): Int =
        if (files[position] != null) FILE_ITEM else SPACING

    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        val file = files[position]
        if (file != null && holder is FileViewHolder) {
